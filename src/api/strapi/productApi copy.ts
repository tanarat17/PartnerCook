...
//         console.log("addProduct FUNCTIONS: ", JSON.stringify(productData, null, 2));
...
//                     image: productData.imageId ? [imageId] : [],
//                 }
//             }),
//         });
...
//         const addedProduct = data.data.attributes;
...
//     console.log('addProduct API : ' + JSON.stringify(productData)); 
...
//         console.log("Respones API Add Product : " + data)
//         return data;
//     } catch (error) {
//         console.error('Error creating Product:', error.message);
//         throw error;
...
export const updateProduct = async (token: string, productId: number, productData: Record<string, any>) => {
    try {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                data: {
                    ...
                    numStock: parseInt(productData.numStock, 10),
                    type: productData.type,
                    shop: { id: productData.shopId }, // ใช้รูปแบบนี้หาก shop เป็น relation
                    image: productData.image ? [{ id: productData.image }] : [],
                }
            }),
        });
        ...
        // ส่งการแจ้งเตือนผ่าน Line Notify หากต้องการ

        return data; // ส่งข้อมูลที่อัปเดตกลับมา
    } catch (error: any) {
