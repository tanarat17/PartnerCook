            ...
            if (productDetails.length > 0 && rawData) {
                ...
                if (matchingProducts.length > 0) {
                    ...
                    if (redeems && Array.isArray(redeems)) {
                        const validRedeems = redeems.filter(redeem =>
                            redeem.status === 'pending' &&
                            redeem.productJsonArray
                        );
                        ...
                        validRedeems.forEach((redeem) => {
                            try {
                                ...
                                if (Array.isArray(productJsonArray)) {
                                    ...
                                    console.error("productJsonArray is not an array:", productJsonArray);
                                }
                            } catch (error) {
                                console.error("Error parsing productJsonArray for redeem:", redeem, error);
                            }
                        });
                    } else {
                    ...
                    <CardContent>
                        ...
                        <TableContainer sx={{ marginBottom: '24px' }}>
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        <TableCell sx={{ textAlign: 'left', fontFamily: 'Sarabun, sans-serif', fontSize: '12px', fontWeight: 'bold' }}>
                                            รายการสินค้า
                                        </TableCell>
                                        <TableCell sx={{ textAlign: 'center', fontFamily: 'Sarabun, sans-serif', fontSize: '12px', fontWeight: 'bold' }}>
                                            จำนวน
                                        </TableCell>
                                        <TableCell sx={{ textAlign: 'left', fontFamily: 'Sarabun, sans-serif', fontSize: '12px', fontWeight: 'bold' }}>
                                            มูลค่า (บาท)
                                ...
                                <TableBody>
                                    {Array.isArray(productDetails) && productDetails.length > 0 ? (
                                        productDetails.map((product, index) => {
                                            ...
                                            return (
                                                <TableRow key={index}>
                                                    <TableCell sx={{ textAlign: 'left', fontFamily: 'Sarabun, sans-serif', fontSize: '12px' }}>
                                                        {product.name}
                                                    </TableCell>
                                                    <TableCell sx={{ textAlign: 'center', fontFamily: 'Sarabun, sans-serif', fontSize: '12px' }}>
                                                        {count}
                                                    </TableCell>
                                                    <TableCell sx={{ textAlign: 'left', fontFamily: 'Sarabun, sans-serif', fontSize: '12px' }}>
                        ...
                        <Typography variant="h6" sx={{ textAlign: 'center', marginBottom: '16px', fontFamily: 'Sarabun', fontSize: '12px', fontWeight: 'bold' }}>
